...
- **输入输出端口**：若同一元件的输入与输出中出现名称、类型完全相同的端口，表示源库中的一个 `VAR_IN_OUT` 端口。生成 FBD JSON 时只在 `portInputs` 中保留一次并设置 `scope: "VAR_IN_OUT"`，同时从 `portOutputs` 排除对应重复项。此判断适用于函数和功能块，不得按元件前缀或特定数据类型硬编码；名称相同但类型不同的端口不得合并。
